...
struct io_manager{
    int (*read)(void *self, char *buf, int offset, int size);
    int (*write)(void *self, char *buf);
    int (*sync)(void *self);
    int (*close)(void *self);
};
...
int file_io_write(void *self, char *buf){
    struct file_io *fio = (struct file_io *)self;
    return write(fio->fd, buf, strlen(buf));
}

int file_io_read(void *self, char *buf, int offset, int size){
    struct file_io *fio = (struct file_io *)self;
...
struct io_manager *create_file_io(char *filepath){
    struct file_io *fio = malloc(sizeof(struct file_io));
